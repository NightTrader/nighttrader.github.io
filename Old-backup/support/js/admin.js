...
(function ($) {
        ...
        openAI: {
            ...
            train: {
                    ...
                    if (url && url.includes('http')) {
                        ...
                        if (url.includes('.xml')) {
                            ...
                            SBF.ajax({ function: 'open-ai-url-training', url: url }, (response) => {
                                if (this.isError(response)) {
                                    return;
                                } else if (!response[0] && response[0][0] !== true) {
                                    this.errors.push(response);
                                }
                                this.website(onSuccess, index + 1);
                            });
                ...
                qea: function (onSuccess) {
                    ...
                    let questions_answers = SBSettings.repeater('get', chatbot_qea_repeater.find('.repeater-item')).map(function (item) { return [item['open-ai-faq-question'], item['open-ai-faq-answer']] });
                    SBF.ajax({
            ...
            init: function () {
                SBF.ajax({
                    ...
                    let code = ['', ''];
                    for (var i = 0; i < response.length; i++) {
                        if (!['sb-conversations', 'sb-articles', 'sb-database'].includes(response[i])) {
                ...
                SBF.ajax({
                    ...
                    let faq = response.map(item => ({ 'open-ai-faq-question': item[0], 'open-ai-faq-answer': item[1] }));
                    if (faq.length) {
                        chatbot_qea_repeater.find('.sb-repeater').html(SBSettings.repeater('set', faq, chatbot_qea_repeater.find('.repeater-item:last-child')));
                    }
                });
            }
        },
    ...
    */
    ...
    var SBUsers = {
        ...
        // Get the users
        get: function (onSusccess, is_online_users = false, is_search_users = false, is_users_pagination = false) {
            let filters = [];
            ...
            loading(users_table);
            SBF.ajax({
                function: is_online_users ? 'get-online-users' : (is_search_users ? 'search-users' : 'get-users'),
    ...
    $(document).ready(function () {
        ...
        $(header).on('click', ' .sb-admin-nav a', function () {
            ...
            switch (id) {
                ...
                case 'sb-chatbot':
                    ...
                    if (SB_ADMIN_SETTINGS.open_ai_chatbot_status !== true) {
                        infoBottom(SB_ADMIN_SETTINGS.open_ai_chatbot_status == 'inactive' ? 'Enable the chatbot in Settings > Artificial Intelligence > OpenAI > Chatbot.' : (SB_ADMIN_SETTINGS.open_ai_chatbot_status == 'key' ? 'Enter the OpenAI API key in Settings > Artificial Intelligence > OpenAI > API key.' : 'Since your chatbot mode is set to Assistant, the training data is ignored.'), 'error');
                    }
            }
        ...
        */
        ...
        $(conversations_area).on('click', '.sb-admin-list .sb-select li', function () {
            ...
            setTimeout(() => {
                ...
                SBF.ajax({
                    ...
                    $(this).closest('.sb-filter-btn').attr('data-budge', conversations_filters.toArray().reduce((acc, filter) => acc + !!$(filter).find('li.sb-active').data('value'), 0));
        ...
        notes_panel.on('click', '> i,.sb-edit-note', function (e) {
            ...
            SBAdmin.genericPanel('notes', note ? 'Edit note' : 'Add new note', `<div class="sb-input-setting sb-type-textarea"><textarea${note ? ' data-id="' + note.attr('data-id') + '"' : ''} placeholder="${sb_('Write here your note...')}">${note ? note.find('.sb-note-text').html().replace(/<a\s+href="([^"]*)".*?>(.*?)<\/a>/gi, '$1').replaceAll('<br>', '\n') : ''}</textarea></div>`, [[note ? 'Update note' : 'Add note', note ? 'check' : 'plus']]);
        ...
        */
        ...
        $(users_filters).on('click', 'li', function () {
            ...
            setTimeout(() => {
                ...
                button.attr('data-budge', users_filters.toArray().reduce((acc, filter) => acc + !!$(filter).find('li.sb-active').data('value'), 0));
            }, 100);
        ...
        */
        ...
        $(chatbot_area).on('click', '.sb-nav [data-value="info"]', function () {
            ...
            SBF.ajax({
                ...
                let list = [['files', 'Files'], ['website', 'Website URLs'], ['qea', 'Q&A'], ['articles', 'Articles'], ['conversations', 'Conversations']];
        ...
        $(chatbot_area).on('click', '.sb-menu-chatbot [data-type]', function () {
            let type = $(this).data('type');
            switch (type) {
                case 'training':
                case 'playground':
                    chatbot_area.find('> [data-id]').sbActive(false)
                    chatbot_area.find(`> [data-id="${type}"]`).sbActive(true);
                    break
                case 'settings':
                    SBSettings.open('dialogflow');
                    break
            }
        });
        ...
        $(chatbot_playground_editor).on('click', '[data-value="add"], [data-value="send"]', function () {
            ...
            if ($(this).data('value') == 'send') {
                if (SBApps.openAI.playground.messages.length && !loading(this)) {
                    SBF.ajax({
            ...
            SBF.search(search, () => {
                ...
                if (search.length > 2) {
                    ...
                    settings_area.find('.sb-setting').each(function () {
                        ...
                        if ((keywords && keywords.includes(search)) || $(this).attr('id').replaceAll('-', '-').includes(search) || $(this).find('.sb-setting-content').text().toLowerCase().includes(search)) {
